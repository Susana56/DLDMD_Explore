...
hyp_params['experiment'] = 'Rossler'
hyp_params['plot_path'] = './training_results/' + hyp_params['experiment'] + 'TRY_CON_' + '_' + hyp_params['sim_start']+ '_' + str(np.log10(gaussian_scale)) + '_noise_scale'
hyp_params['model_path'] = './trained_models/' + hyp_params['experiment'] + 'TRY_CON_' + '_' + hyp_params['sim_start']+ '_' + str(np.log10(gaussian_scale)) + '_noise_scale'
...
hyp_params['time_final'] = 60     # Should I change these
hyp_params['delta_t'] = 0.15
hyp_params['a'] = 0.1      # commonly used parameters for the Rossler attractor
hyp_params['b'] = 0.1
hyp_params['c'] = 14
...
if False: #os.path.exists(data_fname):
    ...
    b = hyp_params['b']
    c = hyp_params['c']
    icx = np.random.uniform(-20, 20, hyp_params['num_init_conds'])
    icy = np.random.uniform(-20, 20, hyp_params['num_init_conds'])
    icz = np.random.uniform(0, 35, hyp_params['num_init_conds'])
    ...
    for ii, ic in enumerate(zip(icx, icy, icz)):
        tmp = solve_ivp(rs, t_span=tspan, y0=ic, method='RK45', t_eval=dts)
        X[ii, :, :] = tmp.y
