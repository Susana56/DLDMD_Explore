...
hyp_params['experiment'] = 'HW_8_4_3'
...
hyp_params['latent_dim'] = 4         # dimension gets projected into
...
if os.path.exists(data_fname):
    ...
    def hw_prob(t, x):
        return [mu * x[0] + x[1] - x[0] ** 2, - x[0] + mu*x[1] + 2*(x[0] ** 2)]
    mu = hyp_params['mu']
    icx = np.random.uniform(-1, 1, hyp_params['num_init_conds'])
    icy = np.random.uniform(-1, 1, hyp_params['num_init_conds'])
    ...
    for ii, ic in enumerate(zip(icx, icy)):
        tmp = solve_ivp(hw_prob, t_span=tspan, y0=ic, method='Radau', t_eval=dts)
        X[ii, :, :] = tmp.y
