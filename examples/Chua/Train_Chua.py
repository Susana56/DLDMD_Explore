...
hyp_params['a'] = 15.395     # commonly used parameters for the chaotic Chua attractor
hyp_params['b'] = 28
...
# code match chaotic attractor produced in https://stackoverflow.com/questions/61127919/chuas-circuit-using-python-and-graphing
# ==============================================================================
# Generate / load data
# ==============================================================================
...
if False: #os.path.exists(data_fname):
    ...
    def rs(t, x):
        ...
        return[a * (x[1] - x[0] - f_x), x[0] - x[1] + x[2], -b*x[1]]
    ...
    for ii, ic in enumerate(zip(icx, icy, icz)):
        tmp = solve_ivp(rs, t_span=tspan, y0=ic, method='LSODA', t_eval=dts)
        X[ii, :, :] = tmp.y
